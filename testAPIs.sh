...
echo
curl -s -X GET \
  "http://localhost:4000/channels/mychannel/chaincodes/mycc?peer=peer0.org1.cc.com&fcn=query&args=%5B%22a%22%5D" \
  -H "authorization: Bearer $org1_TOKEN" \
  -H "content-type: application/json"
echo
echo
...
BLOCK_INFO=$(curl -s -X GET \
  "http://localhost:4000/channels/mychannel/blocks/1?peer=peer0.org1.cc.com" \
  -H "authorization: Bearer $org1_TOKEN" \
  -H "content-type: application/json")
echo $BLOCK_INFO
...
echo
curl -s -X GET http://localhost:4000/channels/mychannel/transactions/$TRX_ID?peer=peer0.org1.cc.com \
  -H "authorization: Bearer $org1_TOKEN" \
  -H "content-type: application/json"
echo
echo
...
echo
curl -s -X GET \
  "http://localhost:4000/channels/mychannel/blocks?hash=$HASH&peer=peer0.org1.cc.com" \
  -H "authorization: Bearer $org1_TOKEN" \
...
echo
curl -s -X GET \
  "http://localhost:4000/channels/mychannel?peer=peer0.org1.cc.com" \
  -H "authorization: Bearer $org1_TOKEN" \
  -H "content-type: application/json"
echo
echo
...
echo
curl -s -X GET \
  "http://localhost:4000/channels/mychannel/chaincodes?peer=peer0.org1.cc.com" \
  -H "authorization: Bearer $org1_TOKEN" \
  -H "content-type: application/json"
echo
echo
